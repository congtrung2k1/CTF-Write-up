...
public class test {
    public static void main(String[] args) throws IOException {
        ...
        //String s = "lollookatthistextigotfromthemanual";
        //byte[] enc = s.getBytes();
        byte[] enc = fillMachine(manualBytes);
        byte[] loadedBytes = strawberryShuffle(vanillaShuffle(chocolateShuffle(toppings(enc))));
        
        System.out.print("flag{");
        for (int i = 0; i < 34; ++i)
            System.out.print((char)loadedBytes[i]);
        System.out.println('}');
    }
    ...
    public static byte[] strawberryShuffle(byte[] inputIceCream) {
        ...
        for (int i = 0; i < outputIceCream.length; i++) {
            outputIceCream[outputIceCream.length - i - 1] = inputIceCream[i];
        }
        return outputIceCream;
    }
        ...
        for (int i = 0; i < outputIceCream.length; i++) {
            if (i % 2 == 0) {
                if (i > 0) {
                    ...
                    outputIceCream[outputIceCream.length - 2] = inputIceCream[i];
                }
            } else {
