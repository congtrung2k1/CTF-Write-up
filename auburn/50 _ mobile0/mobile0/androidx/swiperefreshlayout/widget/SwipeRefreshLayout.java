...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild
{
    ...
    private void moveSpinner(final float a) {
        ...
        final double v = min;
        Double.isNaN(v);
        final float b = (float)Math.max(v - 0.4, 0.0) * 5.0f / 3.0f;
        ...
        final float max = Math.max(0.0f, Math.min(abs - mTotalDragDistance, n2 * 2.0f) / n2);
        final double v2 = max / 4.0f;
        final double pow = Math.pow(max / 4.0f, 2.0);
        Double.isNaN(v2);
        final float n3 = (float)(v2 - pow) * 2.0f;
        ...
        this.mProgress.setProgressRotation((0.4f * b - 0.25f + 2.0f * n3) * 0.5f);
    ...
    protected void onLayout(final boolean b, int measuredWidth, int paddingTop, int n, int measuredHeight) {
        ...
        mTarget.layout(n, paddingTop, n + (measuredWidth - this.getPaddingLeft() - this.getPaddingRight()), paddingTop + (measuredHeight - this.getPaddingTop() - this.getPaddingBottom()));
        ...
        mCircleView.layout(measuredHeight - paddingTop, mCurrentTargetOffsetTop, measuredWidth / 2 + n / 2, mCurrentTargetOffsetTop + measuredHeight2);
    }
    ...
    public void setOnChildScrollUpCallback(final OnChildScrollUpCallback mChildScrollUpCallback) {
    ...
    public void setSlingshotDistance(final int mCustomSlingshotDistance) {
