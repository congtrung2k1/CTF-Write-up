    ...
    void applyTransformation(final float n, final Ring ring, final boolean b) {
        if (this.mFinishing) {
            ...
            if (n < 0.5f) {
                ...
                final float n4 = ring.getStartingStartTrim() + 0.79f;
                final float interpolation = CircularProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation(n3);
                endTrim = n4;
                startingStartTrim = n4 - ((1.0f - interpolation) * 0.79f + 0.01f);
            }
