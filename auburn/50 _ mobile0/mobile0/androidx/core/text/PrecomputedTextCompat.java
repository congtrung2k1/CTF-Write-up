...
public class PrecomputedTextCompat implements Spannable
{
    ...
    public static PrecomputedTextCompat create(final CharSequence charSequence, final Params params) {
        ...
        try {
            ...
            final PrecomputedTextCompat precomputedTextCompat = new PrecomputedTextCompat(charSequence, params, array);
    ...
    private int findParaIndex(final int i) {
        int n = 0;
        while (true) {
            final int[] mParagraphEnds = this.mParagraphEnds;
            if (n >= mParagraphEnds.length) {
                final StringBuilder sb = new StringBuilder();
                sb.append("pos must be less than ");
                final int[] mParagraphEnds2 = this.mParagraphEnds;
                sb.append(mParagraphEnds2[mParagraphEnds2.length - 1]);
                sb.append(", gave ");
                sb.append(i);
                throw new IndexOutOfBoundsException(sb.toString());
            }
            if (i < mParagraphEnds[n]) {
                return n;
            }
            ++n;
        }
