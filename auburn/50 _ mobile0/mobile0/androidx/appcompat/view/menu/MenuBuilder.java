...
public class MenuBuilder implements SupportMenu
{
    ...
    private boolean dispatchSubMenuSelected(final SubMenuBuilder subMenuBuilder, MenuPresenter menuPresenter) {
        if (this.mPresenters.isEmpty()) {
            return false;
        }
        int onSubMenuSelected = 0;
        if (menuPresenter != null) {
            onSubMenuSelected = (menuPresenter.onSubMenuSelected(subMenuBuilder) ? 1 : 0);
        }
        for (final WeakReference<MenuPresenter> o : this.mPresenters) {
            menuPresenter = (MenuPresenter)o.get();
            int onSubMenuSelected2;
            if (menuPresenter == null) {
                this.mPresenters.remove(o);
                onSubMenuSelected2 = onSubMenuSelected;
            }
            else {
                onSubMenuSelected2 = onSubMenuSelected;
                if (onSubMenuSelected == 0) {
                    onSubMenuSelected2 = (menuPresenter.onSubMenuSelected(subMenuBuilder) ? 1 : 0);
                }
            }
            onSubMenuSelected = onSubMenuSelected2;
        }
        return onSubMenuSelected != 0;
    }
    ...
    public boolean collapseItemActionView(final MenuItemImpl menuItemImpl) {
        if (!this.mPresenters.isEmpty() && this.mExpandedItem == menuItemImpl) {
            ...
            while (true) {
                ...
                else if (collapseItemActionView = menuPresenter.collapseItemActionView(this, menuItemImpl)) {
    ...
    public boolean expandItemActionView(final MenuItemImpl mExpandedItem) {
        ...
        while (true) {
            ...
            else if (expandItemActionView = menuPresenter.expandItemActionView(this, mExpandedItem)) {
    ...
    void findItemsWithShortcutForKey(final List<MenuItemImpl> list, final int n, final KeyEvent keyEvent) {
        ...
        for (int size = this.mItems.size(), i = 0; i < size; ++i) {
            ...
            if ((modifiers & 0x1100F) == (0x1100F & n2) && c != '\0' && (c == keyCharacterMap$KeyData.meta[0] || c == keyCharacterMap$KeyData.meta[2] || (qwertyMode && c == '\b' && n == 67)) && menuItemImpl.isEnabled()) {
    ...
    public boolean performItemAction(final MenuItem menuItem, final MenuPresenter menuPresenter, final int n) {
        ...
        if (menuItemImpl != null && menuItemImpl.isEnabled()) {
            ...
            if (menuItemImpl.hasCollapsibleActionView()) {
                ...
                b3 = (invoke | this.dispatchSubMenuSelected(subMenuBuilder, menuPresenter));
                if (!b3) {
                    this.close(true);
                }
            }
