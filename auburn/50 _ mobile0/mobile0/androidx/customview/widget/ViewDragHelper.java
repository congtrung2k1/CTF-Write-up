...
public class ViewDragHelper
{
    ...
    private float clampMag(float a, final float n, final float n2) {
    ...
    private int clampMag(int a, final int n, final int n2) {
    ...
    private int computeAxisDuration(int a, int abs, final int n) {
        ...
        abs = Math.abs(abs);
        if (abs > 0) {
            a = Math.round(Math.abs((n3 + n4 * distanceInfluenceForSnapDuration) / abs) * 1000.0f) * 4;
        }
        else {
            a = (int)((1.0f + Math.abs(a) / (float)n) * 256.0f);
        }
            ...
            if (actionMasked != 1) {
                if (actionMasked != 2) {
                    ...
                    if (this.isValidPointerForActionMove(this.mActivePointerId)) {
                        ...
                        final int n4 = (int)(x2 - mLastMotionX[mActivePointerId2]);
                        final int n5 = (int)(y2 - this.mLastMotionY[mActivePointerId2]);
                    ...
                    for (int pointerCount2 = motionEvent.getPointerCount(), i = 0; i < pointerCount2; ++i) {
                        ...
                        if (this.isValidPointerForActionMove(pointerId4)) {
                            ...
                            final float n6 = x3 - this.mInitialMotionX[pointerId4];
                            final float n7 = y3 - this.mInitialMotionY[pointerId4];
                            this.reportNewEdgeDrags(n6, n7, pointerId4);
                            if (this.mDragState == 1) {
                                break;
                            }
                            final View topChildUnder2 = this.findTopChildUnder((int)x3, (int)y3);
                            if (this.checkTouchSlop(topChildUnder2, n6, n7) && this.tryCaptureViewForDrag(topChildUnder2, pointerId4)) {
                                break;
                            }
                        }
                    }
